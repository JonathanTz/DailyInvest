...
# 爬取下市下櫃資訊至DB
def cra_DelistInfo(conn, pDate): # 💡 修正原本傳入 con 卻在裡面用 conn 的問題
    my_data = {'stk_code':'' , 'select_year': 'ALL', 'DELIST_REASON': -1, 'topage':1}
...
def covtSecPxToDB(conn, sec, dtStart, nDay=-60):
    ...
    if jFile['statusCode'] == 200:
        ...
        # 💡 修正：原本傳入 conn2 未定義，因為你使用了 ATTACH，直接用主連線 conn 寫入即可
        df_stkInfo.to_sql(
            name="securityPrice", 
            con=conn, 
            if_exists='append', 
...
def getSecID(con, sec):
    cur = con.cursor()
    # 💡 修正：當你在 get_connection(1) 使用了 ATTACH AS MDEngine 時
    # 這裡的寫法應該是 MDEngine.securityInfo（大寫跟著當時 ATTACH 名稱）
...
def run_crawler(): 
    ...
    conn = get_connection(1)  # 包含 FP 與 MDEngine (Attached)
    conn2 = get_connection(2) # 僅 MDEngine
    ...
    for ticker in aStk:
        # 💡 傳入帶有 MDEngine 的連線 conn
        covtSecPxToDB(conn, ticker, dtNow, -100)

    end_time = time.time()
    print(f"爬蟲任務完成，耗時：{end_time - start_time:.2f} 秒")

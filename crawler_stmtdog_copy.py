...
def scrape_stmt(urls):
    ...
    try :
        ...
        scrape_stmt(urls)
def run_scrape_mkt(nThread=5):
    base_url = "https://statementdog.com/api/v2/fundamentals"
    listStk = [f"{str(stkID).zfill(4)}" for stkID in range(8000, 9999)]#[3089,3609,4431,5321,5392,5432,5529,5859,6468,6721,7419,7421,8705]
    #listStk = [1472,2340,2443,2736,4154,5604,6171,6517,6831,6840]
    urls = [f"{base_url}/{str(stkID).zfill(4)}/2016/2023/cf?qbu=true&qf=analysis" for stkID in listStk]  # 1~5頁的網址清單
    with concurrent.futures.ThreadPoolExecutor(max_workers=nThread) as executor:
        executor.map(scrape_stmt, urls)

def run_scrape_recover(arFile):
    base_url = "https://statementdog.com/api/v2/fundamentals"
    ii = [f"{str(stkID).zfill(4)}" for stkID in range(8000, 9999)]
    new_urls = [str(i)[:4] for i in arFile]
    for j in ii:
        if j not in new_urls:
            scrape_stmt(base_url+"/"+str(j).zfill(4)+"/2016/2023/cf?qbu=true&qf=analysis")
 

start_time = time.time()  # 開始時間
#----------------- 1 . 抓取財報狗財報-同時建立及啟用5個執行緒
run_scrape_mkt()

#----------------- 2 . 抓取財報狗財報-回補遺漏資料(上述多執行緒缺漏)
mypath='raw/'  
files = listdir(mypath)
run_scrape_recover(files)
...
df_stkInfo.to_csv("df_stkInfo_202302.csv",encoding='utf-8')
